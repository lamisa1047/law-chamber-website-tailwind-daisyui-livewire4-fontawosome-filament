
<div class="bg-gray-100">


<!-- ══════════ CONTACT SECTION ══════════ -->
<section class="py-12 sm:py-16">
  <div class="max-w-7xl mx-auto px-4 sm:px-6">

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">

      <!-- ═══ Contact Form ═══ -->
      <div class="lg:col-span-2 bg-white p-6 sm:p-8 rounded-2xl shadow-sm">

        <h2 class="text-2xl font-bold text-forest mb-6">Send us a message</h2>

        <form class="space-y-5">

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input type="text" placeholder="Your Name"
              class="input input-bordered w-full"/>

            <input type="email" placeholder="Your Email"
              class="input input-bordered w-full"/>
          </div>

          <input type="text" placeholder="Subject"
            class="input input-bordered w-full"/>

          <textarea placeholder="Your Message"
            class="textarea textarea-bordered w-full h-32"></textarea>

          <button type="submit"
            class="px-6 py-3 bg-gold text-white font-semibold rounded-lg hover:bg-gold-dark transition w-full sm:w-auto">
            Send Message
          </button>

        </form>
      </div>

      <!-- ═══ Contact Info ═══ -->
      <div class="space-y-5">

        <div class="bg-white p-6 rounded-2xl shadow-sm">
          <div class="flex items-center gap-4">
            <div class="w-12 h-12 flex items-center justify-center rounded-full bg-gold text-white">
              <i class="fas fa-phone"></i>
            </div>
            <div>
              <h4 class="font-bold text-forest">Phone</h4>
              <p class="text-sm text-gray-500">+880 1234 567890</p>
            </div>
          </div>
        </div>

        <div class="bg-white p-6 rounded-2xl shadow-sm">
          <div class="flex items-center gap-4">
            <div class="w-12 h-12 flex items-center justify-center rounded-full bg-forest text-white">
              <i class="fas fa-envelope"></i>
            </div>
            <div>
              <h4 class="font-bold text-forest">Email</h4>
              <p class="text-sm text-gray-500">info@ensaflaw.com</p>
            </div>
          </div>
        </div>

        <div class="bg-white p-6 rounded-2xl shadow-sm">
          <div class="flex items-center gap-4">
            <div class="w-12 h-12 flex items-center justify-center rounded-full bg-gold text-white">
              <i class="fas fa-map-marker-alt"></i>
            </div>
            <div>
              <h4 class="font-bold text-forest">Address</h4>
              <p class="text-sm text-gray-500">
                Khulna, Bangladesh
              </p>
            </div>
          </div>
        </div>

      </div>

    </div>

    <!-- ═══ MAP ═══ -->
    <div class="mt-12 rounded-2xl overflow-hidden shadow-sm">
      <iframe
        src="https://www.google.com/maps?q=Khulna,Bangladesh&output=embed"
        width="100%" height="350" style="border:0;" loading="lazy">
      </iframe>
    </div>

  </div>
</section>

</div>